...
parser.add_argument(
    "--dataset", default="BP4D", type=str, help="database"
)  # BP4D,DISFA
parser.add_argument("--dataset_test", default="demo", type=str)  # BP4D-val, DISFA-val
parser.add_argument(
    "--model_path", type=str, default="./model/model.pth", help="model path"
)  # tuned_model.pth
    ...
    with torch.no_grad():
        ...
        for sample in loader:
            ...
            for (index, item) in enumerate(sample["Im"].to("cpu").detach()):
                ...
                heatmap_AU10_0_np = (
                    (heatmap_AU10_0.numpy() / 5.0).astype(np.uint8).copy()
                )
                ...
                heatmap_AU10_1_np = (
                    (heatmap_AU10_1.numpy() / 5.0).astype(np.uint8).copy()
                )
                ...
                heatmap_AU12_0_np = (
                    (heatmap_AU12_0.numpy() / 5.0).astype(np.uint8).copy()
                )
                ...
                heatmap_AU12_1_np = (
                    (heatmap_AU12_1.numpy() / 5.0).astype(np.uint8).copy()
                )
                ...
                heatmap_AU14_0_np = (
                    (heatmap_AU14_0.numpy() / 5.0).astype(np.uint8).copy()
                )
                ...
                heatmap_AU14_1_np = (
                    (heatmap_AU14_1.numpy() / 5.0).astype(np.uint8).copy()
                )
                ...
                heatmap_AU17_0_np = (
                    (heatmap_AU17_0.numpy() / 5.0).astype(np.uint8).copy()
                )
                ...
                heatmap_AU17_1_np = (
                    (heatmap_AU17_1.numpy() / 5.0).astype(np.uint8).copy()
                )
            ...
            save_AU6 = torch.nn.functional.interpolate(
                torch.from_numpy(maps_AU6 / 255.0).permute(0, 3, 1, 2), scale_factor=0.5
            )
            save_image(save_AU6, "./visualize/Subject{}_AU06.png".format(count))
            save_AU10 = torch.nn.functional.interpolate(
                torch.from_numpy(maps_AU10 / 255.0).permute(0, 3, 1, 2),
                scale_factor=0.5,
            )
            save_image(save_AU10, "./visualize/Subject{}_AU10.png".format(count))
            save_AU12 = torch.nn.functional.interpolate(
                torch.from_numpy(maps_AU12 / 255.0).permute(0, 3, 1, 2),
                scale_factor=0.5,
            )
            save_image(save_AU12, "./visualize/Subject{}_AU12.png".format(count))
            save_AU14 = torch.nn.functional.interpolate(
                torch.from_numpy(maps_AU14 / 255.0).permute(0, 3, 1, 2),
                scale_factor=0.5,
            )
            save_image(save_AU14, "./visualize/Subject{}_AU14.png".format(count))
            save_AU17 = torch.nn.functional.interpolate(
                torch.from_numpy(maps_AU17 / 255.0).permute(0, 3, 1, 2),
                scale_factor=0.5,
            )
...
def test_epoch(dataset_test, model_path, size, npoints):
    ...
    dbloader = DataLoader(
        database, batch_size=1, shuffle=False, num_workers=0, pin_memory=False
    )
